...
void aimbot() {
	...
	while (true) {
		Sleep(5);
		if (Config::config.aim == false) continue;
		for (int i = 0; i < 30; ++i) {
			LocalPlayer.Read();
		...
		if (GetAsyncKeyState(0x01)) {
			...
			if (Aimbot::GetFOV(LocalPlayer.position, EntityList.position, LocalPlayer.viewAngles) < Config::config.fov) {
...
void renderEsp(HWND hWnd) {
	Drawer::SetupDrawing(hWnd);
	...
	for (int i = 0; i < 32; ++i) {
		...
		if (EntityList.alive != 3)continue;
		if (EntityList.Dormant != 1) continue;
		ESP::Render(LocalPlayer, EntityList);
		
	}
	Drawer::EndDrawing(hWnd);
}
...
void updateOverlay(HWND overlayHwnd) {
	HWND csHWND = FindWindow(NULL, L"Counter-Strike");
	RECT rSize;
	GetWindowRect(csHWND, &rSize);
	int Width = rSize.right - rSize.left;
	int Height = rSize.bottom - rSize.top;
	DWORD dwStyle = GetWindowLong(csHWND, GWL_STYLE);
	if (dwStyle & WS_BORDER) {
		rSize.top += 23;
		Height -= 23;
	}

	MoveWindow(overlayHwnd, rSize.left, rSize.top, Width, Height, true);
}

void Glassfish(HWND hWnd) {
	HWND csHWND = FindWindow(NULL, L"Counter-Strike");
	RECT rSize;
	GetWindowRect(csHWND, &rSize);
	int Width = rSize.right - rSize.left;
